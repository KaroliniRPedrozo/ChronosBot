...
with engine.connect() as conn:
    ...
    for row in conn.execute(text("select constraint_name, constraint_type from information_schema.table_constraints where table_name='materiais'")):
        print(row)
    print('COLUMN EXISTS DETAIL:')
    for col in ['turma_id','processado','data_liberacao']:
        res = conn.execute(text("select column_name, is_nullable from information_schema.columns where table_name='materiais' and column_name=:col"), {'col': col}).fetchall()
        print(col, res)
